...
public enum EComponent {
    ...
    public ILamarkComponent getComponent(Lamark lam) {
        switch (this) {
            case CREATOR:
                return lam.getCreator();
            case CROSSOVER:
                return lam.getCrossover();
            case FITNESSFUNCTION:
                return lam.getFitnessFunction();
            case MUTATOR:
                return lam.getMutator();
            case SELECTOR:
                return lam.getSelector();
            default:
                throw new IllegalStateException("Cant happen: invalid enum:" + this);
        }
    }
    ...
    public String getClassProperty() {
        switch (this) {
            case CREATOR:
                return CREATOR_CLASS_KEY;
            case CROSSOVER:
                return CROSSOVER_CLASS_KEY;
            case FITNESSFUNCTION:
                return FITNESS_CLASS_KEY;
            case MUTATOR:
                return MUTATOR_CLASS_KEY;
            case SELECTOR:
    ...
    public String getPropertyPrefix() {
        switch (this) {
            case CREATOR:
                return CREATOR_PREFIX;
            case CROSSOVER:
                return CROSSOVER_PREFIX;
            case FITNESSFUNCTION:
                return FITNESS_PREFIX;
            case MUTATOR:
